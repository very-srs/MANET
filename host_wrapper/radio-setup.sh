...
#echo "mesh_hardware=__AP_HARDWARE__" >> /etc/mesh.conf #ax ah both none
...
if [[ -n "$acs" ]]; then
	if [[ "$acs" == "Y" ]]; then
		echo " > Automatic channel selection / hoping enabled"
		mv /usr/local/bin/node-manager-acs.sh /usr/local/bin/node-manager.sh
	else
		echo " > Static channel selection will be used"
		mv /usr/local/bin/node-manager-static.sh /usr/local/bin/node-manager.sh
	fi
...
if [[ -n "$mtx" ]]; then
	if [[ "$mtx" == "Y" ]]; then
	echo " > This mesh will host a mediaMTX server ..."
	systemctl enable mediamtx
fi

if [[ -n "$mumble" ]]; then
	if [[ "$mtx" == "Y" ]]; then
	echo " > This mesh will host a mumble server ..."
	systemctl enable mumble
fi
